...
const Search = () => {
  ...
  useEffect(() => {
    if (selectedOption && selectedOption.value === "codigo") {
      fetch(`${principalURL}/campus/codigou/${codigo}`)
      ...
      const formattedEndDate = endDate
        ...
        .replace(/(\d{2})\/(\d{2})\/(\d{4}),\s/, "$3-$2-$1T");
      fetch(
        `${principalURL}/campus/fechaingreso?fechaInicial=${formattedStartDate}&fechaFinal=${formattedEndDate}`
      )
        .then((response) => response.json())
        .then((data) => setData(data))
        .catch((error) => setError(error.toString()));
      ...
      const formattedEndDate = endDate
        ...
        .replace(/(\d{2})\/(\d{2})\/(\d{4}),\s/, "$3-$2-$1T");
      fetch(
        `${principalURL}/campus/idcodigouandfechaingreso?idCodigoU=${codigo}&fechaInicial=${formattedStartDate}&fechaFinal=${formattedEndDate}`
      )
        .then((response) => response.json())
        .then((data) => setData(data))
        .catch((error) => setError(error.toString()));
