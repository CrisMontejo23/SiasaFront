...
const Search = () => {
  ...
  useEffect(() => {
    const fetchWithDates = (endpoint) => {
      ...
      fetch(
        `${principalURL}/salacomputo/${endpoint}?fechaInicial=${formattedStartDate}&fechaFinal=${formattedEndDate}`
      )
    ...
    const fetchWithDatesAndCode = (endpoint) => {
      ...
      const formattedEndDate = formatDates(endDate);
      fetch(
        `${principalURL}/salacomputo/${endpoint}?idCodigoU=${codigo}&fechaInicial=${formattedStartDate}&fechaFinal=${formattedEndDate}`
      )
        .then((response) => response.json())
        .then((data) => setData(data))
        .catch((error) => setError(error.toString()));
    ...
    if (selectedOption && selectedOption.value === "codigo") {
      fetch(`${principalURL}/salacomputo/codigou/${codigo}`)
        .then((response) => response.json())
        .then((data) => setData(data))
        .catch((error) => setError(error.toString()));
